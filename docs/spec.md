...
11. Edge cache layer: check the Workers Cache API (`caches.default`) before the R2 logic above. On an edge hit, return the cached response. On an edge miss, run steps 3–8, then write the response to the edge cache via `ctx.waitUntil` (non-blocking). The cache key is the request URL, which is sound because versioned URLs are immutable. See ADR `docs/decisions/0004-edge-cache-and-cache-status.md`.
12. Cache-status observability headers on image responses:
    - `X-Cache: HIT | MISS` — whether the edge cache (`caches.default`) served the response.
    - `X-Cache-Source: edge | r2 | images` — provenance of the bytes: edge cache, a persisted R2 object/variant, or a fresh `IMAGES` transform.
